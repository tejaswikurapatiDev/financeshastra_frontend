...
const ReferMoreProfilePages = () => {
    ...
    const validateField = (input) => {
      ...
      const isEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email) && input.email.endsWith('.com');
      if (!input.email) errors.email = 'Email is required';
      else if (!isEmailValid) errors.email = 'Invalid email. It should end with .com';
      ...
      else if (input.mobileNumber.length < 10) errors.mobileNumber = 'Mobile number must be at least 10 digits';
    ...
    const isFormValid = inputs.every(input => !input.errors || Object.keys(input.errors).length === 0);
  ...
  <a href="#" className="myreferalpagesss-benefits-link">View benefits</a>
        ...
        <div className="myreferalpagesss-buttons">
          ...
          <button className="myreferalpagesss-button active"onClick={() => navigate("/referMoreProfilePages")}>Refer More</button>
  ...
  {/* "OR" Section */}
  <div className="refermoreprofilepages-or">
    OR
  </div>
  ...
  {inputs.map((input, index) => {
    const validationResults = validateField(input);
    return (
      <div key={index} className="refermoreprofilepages-input-group">
        {/* First Name */}
        <div
          className={`refermoreprofilepages-floating-input ${input.touched && validationResults.firstName ? 'error' : ''}`}
        >
        ...
        {/* Last Name */}
        <div
          className={`refermoreprofilepages-floating-input ${input.touched && validationResults.lastName ? 'error' : ''}`}
        >
        ...
        {/* Mobile Number */}
        <div
          className={`refermoreprofilepages-floating-input ${input.touched && validationResults.mobileNumber ? 'error' : ''}`}
        >
        ...
        {/* Email */}
        <div
          className={`refermoreprofilepages-floating-input ${input.touched && validationResults.email ? 'error' : ''}`}
        >
...
<h3 className="refermoreprofilepagess-email-templateh3">Kindly follow the email format provided below.</h3>
