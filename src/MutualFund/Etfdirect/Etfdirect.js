...
const Etfdirect = () => {
  ...
  const totalPages = Math.max(1, Math.ceil((fundDatadirectetf?.length || 0) / recordsPerPage));
  ...
  const indexOfLastItem = Math.min(indexOfFirstItem + recordsPerPage, fundDatadirectetf.length);
      ...
      <div className="pagination-topratedcontainer">
        <div className="pagination-topratedwrapper">
          <div className="pagination-topratedinfo">
            {`Showing ${indexOfFirstItem + 1} to ${indexOfLastItem} of ${fundDatadirectetf.length} records`}
          </div>
