...
const Bestgrowthregular = () => {
 
    const navigate = useNavigate();
    const [sortConfig, setSortConfig] = useState({ key: null, direction: "asc" });
  
    const sortedData = () => {
      if (!sortConfig.key) return fundDataregulargrowth;
  
      const sorted = [...fundDataregulargrowth];
      sorted.sort((a, b) => {
...
export default Bestgrowthregular;