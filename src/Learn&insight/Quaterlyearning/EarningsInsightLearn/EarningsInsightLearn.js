...
const EarningsInsightLearn = () => {
  ...
  const handleFilterChange = useCallback(
    (selectedFilter) => {
      ...
      if (selectedFilter === "Upcoming Results") {
        setSortedData(
          earningsData.filter((row) => row.Type === "Upcoming Results")
        );
      } else if (selectedFilter === "Declared Results") {
        setSortedData(
          earningsData.filter((row) => row.Type === "Declared Results")
        );
      } else {
  ...
  const handleMcapOptionChange = useCallback(
    (option) => {
      ...
      const sorted = [...sortedData].sort((a, b) => {
        if (option === "Market Cap") {
          const valueA = parseFloat(
            a.mcap.replace("₹", "").replace(",", "").replace(" Cr", "")
          );
          const valueB = parseFloat(
            b.mcap.replace("₹", "").replace(",", "").replace(" Cr", "")
          );
    ...
    (column) => {
      ...
      const sorted = [...sortedData].sort((a, b) => {
        if (column === "ltp" || column === "revenue" || column === "mcap") {
          const valueA = parseFloat(
            a[column].replace("₹", "").replace(",", "")
          );
          const valueB = parseFloat(
            b[column].replace("₹", "").replace(",", "")
          );
          return newSortOrder === "asc" ? valueA - valueB : valueB - valueA;
        }
        return newSortOrder === "asc"
    ...
    (dates) => {
      ...
      if (start && end) {
        const filteredData = earningsData.filter((row) => {
          const rowDate = new Date(row.tentativeTime)
            .toISOString()
            .split("T")[0];
          return (
            rowDate >= start.toISOString().split("T")[0] &&
    ...
    try {
      ...
      if (response.ok === true) {
        ...
        if (quaterlyarticles[0]) {
          ...
          navigate(`/earningsresult/${encodedTitle}`, {
    ...
    <div>
      <div className="earnings-insight-learn-wrapper">
        ...
        <header className="earnings-insight-learn-header">
          <h1 className="earnings-insight-learnh1">
            Quarterly Earning Results
          </h1>
          <p className="earnings-insight-learnp">
            Looking for the best growth funds to accelerate your wealth
            creation? At Value Research, we've made the process easier for you.
            Our in-depth guide to top-performing growth funds
            <br />
            across different categories helps you find options that align with
            your long-term financial goals and maximize your potential for
            growth.
          </p>
        </header>
          ...
          <table className="earnings-insight-learn-table">
            <thead>
              <tr>
                ...
                <th
                  onClick={() => handleSort("tentativeTime")}
                  style={{ cursor: "pointer" }}
                >
                  ...
                  <td>
                    <Link
                      to={{
                        ...
                        search: `?company_id=${
                          row.company_id
                        }&company_name=${encodeURIComponent(row.company)}/${
                          row.tag
                        }`,
                      }}
                    >
